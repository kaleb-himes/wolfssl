...
//static int test_chainG_and_chainH(WOLFSSL_CERT_MANAGER* cm)
//{
//    int ret;
//    int i = -1;
//    /* Chain G is a valid chain per RFC 5280 section 4.2.1.9 */
...
//    VERIFY_ONE_CERT(ret, i, cm, chainGArr[8]); /* if failure, i = -16 here */
//
//    VERIFY_ONE_CERT(ret, i, cm, chainHArr[5]); /* if failure, i = -13 here */
//    VERIFY_ONE_CERT(ret, i, cm, chainHArr[1]); /* if failure, i = -9 here */
//    VERIFY_ONE_CERT(ret, i, cm, chainHArr[2]); /* if failure, i = -10 here */
//    VERIFY_ONE_CERT(ret, i, cm, chainHArr[3]); /* if failure, i = -11 here */
//    VERIFY_ONE_CERT(ret, i, cm, chainHArr[4]); /* if failure, i = -12 here */
//
///* test validating the entity twice, should have no effect on pathLen since
// * entity/leaf cert */
//    VERIFY_ONE_CERT(ret, i, cm, chainGArr[8]); /* if failure, i = -17 here */
