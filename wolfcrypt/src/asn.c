...
printf("KH:0 Setting default pathLen!!!\n");
            cert->pathLength = cert->maxPathLen = WOLFSSL_MAX_PATH_LEN;
            cert->pathLength = cert->maxPathLenSet = 1;
        }
        ...
        if (verify != NO_VERIFY && type != CA_TYPE &&
            ...
            if (cert->ca) {
                if (cert->isCA) {
                    ...
                    if (cert->extKeyUsageSet) {
                         ...
                         if ((cert->extKeyUsage & KEYUSE_KEY_CERT_SIGN) != 0) {
                              WOLFSSL_MSG("\tKey Usage includes cert signing");
                              WOLFSSL_MSG("\tCheck for PathLen");
                              checkPathLen = 1;
                         } else {
                              WOLFSSL_MSG("\tKey Usage does not include cert "
                                          "signing, reduce max path by 1");
                ...
                printf("KH: Working on cert: %s\n", cert->subject);
                printf("KH-BEFORE: cert->ca->maxPathLen = %d\n", cert->ca->maxPathLen);
                printf("KH-BEFORE: cert->ca->pathLength = %d\n", cert->ca->pathLength);

                if (checkPathLen && cert->pathLengthSet) {
                    if (cert->pathLength < cert->ca->maxPathLen) {
                printf("KH:1.a, reduce to certs path length\n");
                        WOLFSSL_MSG("\treduce max path to pathLen constraint");
                        cert->ca->maxPathLen = cert->pathLength;
                    } else {
                        WOLFSSL_MSG("\tpathLen constraint not set, reduce max path"
                                    " by 1");
                printf("KH:1.b, reduce by only 1\n");
                        reduceMaxPathByOne = 1;
                    }
                }

                if (reduceMaxPathByOne && cert->ca->maxPathLen > 0) {
                    WOLFSSL_MSG("\tmax path reduced by 1");
                printf("KH:1.c simply reduce by 1\n");
                    cert->ca->maxPathLen -= 1;
                } else if (reduceMaxPathByOne && cert->ca->maxPathLen <= 0) {
                printf("KH-ERROR CONDITION HIT\n");
                    WOLFSSL_MSG("\tNon-entity cert encountered and maxPathLen is"
                                " 0");
                    return ASN_PATHLEN_INV_E;
                }
                printf("KH-AFTER: cert->ca->maxPathLen = %d\n", cert->ca->maxPathLen);
                printf("KH-AFTER: cert->ca->pathLength = %d\n", cert->ca->pathLength);
                printf("KH-----------------------------------------------------------------\n");
